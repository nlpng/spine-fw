...
EventQueue printfQueue(32 * EVENTS_EVENT_SIZE);
...
void spi_isr(void)
{
    ...
    if(calc_checksum != spi_command.checksum) {
        printfQueue.call(&printf_debug, "Incorrect checksum");
        spi_data.flags[0] = 0xdead;
        spi_data.flags[1] = 0xdead;
    }

    eventQueue.call(&control);
}
